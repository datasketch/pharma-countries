...
#
#     if (is.null(actual_but$active)) return()
#     if (actual_but$active == "bar")   sliderInput("sel_slide_opts","Number of ATC to display",min=10, max=20, value=c(1,20))
#
#   })
#   sel_slide_opts_max <- reactive({
#     require(data_viz())
#
#     if (is.null(actual_but$active)) return()
#     if (actual_but$active == "bar") val <- nrow(data_viz())
#   })
#
#
#
...
#         }
#
#         if (actual_but$active == "bar") {   opts$order <- c("mean") } # no ha dado el resultado esperado
#         if (actual_but$active == "treemap") {
#           opts$dataLabels_align <- "middle"
#           opts$dataLabels_inside <- TRUE
#           opts$dataLabels_show <- TRUE
